...
if(isset($_REQUEST['id'])){
 $id = $_REQUEST['id'];
$sql = "SELECT DISTINCT a.id as att_id,a.date,a.duration,a.user_id,a.emp_id,a.emp_name,a.emp_dept,s.venue,a.admin_id,s.name FROM `attendance` a INNER JOIN add_details s ON a.admin_id=s.id WHERE a.admin_id = '".$id."' ORDER BY a.date";
...
<table class="<?php if($k!=1){ echo 'pagebreak'; } ?>" cellspacing="0" border="0">
	...
	<tr>
		...
		<td style="border-top: 1px solid #000000; border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000" align="center" valign=middle><b><font face="Times New Roman" size=3 color="#000000">Edit</font></b></td>
		<td style="border-top: 1px solid #000000; border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000" align="center" valign=middle><b><font face="Times New Roman" size=3 color="#000000">Delete</font></b></td>
	</tr>
<?php } ?>
	<tr>
		...
		<td style="border-top: 1px solid #000000; border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000" align="center" valign=middle><font face="Times New Roman" size=3 color="#000000">
		<a href="attendance_report_edit.php?id=<?php echo $a['att_id'] ?>&form=<?php echo $a['admin_id'] ?>" > 
                              <button class="btn btn-secondary btn-sm" data-toggle="tooltip" data-original-title="Edit" > Edit </button>         
                            </a>
	   </font></td>
		<td style="border-top: 1px solid #000000; border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000" align="center" valign=middle><font face="Times New Roman" size=3 color="#000000">
		<a href="javascript:void(0);" onclick="deleteAttendance('<?php echo $a['att_id'] ?>','<?php echo $a['admin_id'] ?>');"> 
                              <button class="btn btn-secondary btn-sm" data-toggle="tooltip" data-original-title="Delete" > Delete </button>         
                            </a>
	    </font></td>
...
<!-- ************************************************************************** -->
<script>
  function deleteAttendance(id,form) {
        var r = confirm("Do you want to delete Attendance ! this action can't be undone.");
        if (r == true) {   
          alert('Attendance deleted Successfully !');
          document.location.href='delete_attendance.php?dlt_id='+id+'&form='+form;
        } 
