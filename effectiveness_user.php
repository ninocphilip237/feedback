...
$sql = "SELECT * from add_details where status=1";
    ...
    <div class="d-flex align-items-stretch">
      ...
      <nav id="sidebar">
        ...
        <ul class="list-unstyled">
          ...
          <li class="active" ><a href="effectiveness_user.php"> <i class="icon-chart"></i>Effectiveness Feedback</a></li>
      ...
      <div class="page-content">
        ...
        <div class="page-header no-margin-bottom">
          <div class="container-fluid">
            <h2 class="h5 no-margin-bottom">Trainer Feedback</h2>
          </div>
        </div>
        <!-- Breadcrumb-->
        <div class="container-fluid">
                ...
                <div class="block">
                  ...
                  <div class="">
                    <table class="table table-striped">
                      ...
                      <tbody>
                      <?php
while ($t_data = mysqli_fetch_array($exe)) {

  // $sql1 = "SELECT * from feedback_effectiveness where user_id='".$t_data['fac_id']."' and learning_tp IS NOT NULL";
  // $exe1 = mysqli_query($conn, $sql1);
  // print_r($exe1->num_rows);
  ...
  //  while ($row1 = mysqli_fetch_array($exe1)) {
    ...
    $firstDate = strtok($t_data['date'], ",");
    if(strtotime($firstDate) < strtotime('-90 days')) {
         ?>
                     
                        <tr>
                        <td> 
                            <?php 
                              $sql1 = "SELECT * from feedback_effectiveness where admin_id='".$t_data['id']."' and learning_tp IS NOT NULL";
                              $exe1 = mysqli_query($conn, $sql1);
                              $row1 =  $exe1->fetch_assoc();
                              // print_r($row1);
                              $total = 0;
                              $sql3 = "SELECT * From feedback where feedback.admin_id='".$t_data['id']."'";
                              $exe3=mysqli_query($conn,$sql3);
                              foreach($exe3 as $t) {
                                $sqlz = "SELECT * from attendance where user_id='".$t['user_id']."' and admin_id='".$t['admin_id']."'";
                                $exez = mysqli_query($conn, $sqlz);
                                if($exez->num_rows > 0){
                                  $total++;
                                }
                              }
                            if($exe1->num_rows == $total){ ?>
                               <a href="print_effectiveness_user.php?feedback_id=<?php echo $t_data['id'] ?>" target="_blank">View <?php echo $exe1->num_rows; ?> Response(s)</a>

                            <?php } else {  ?>
                              <a href="effectiveness_submit.php?feedback_id=<?php echo $t_data['id'] ?>"> Submit Response (<?php echo $total - $exe1->num_rows;  ?> Pending) </a>
                            </br> <a href="print_effectiveness_user.php?feedback_id=<?php echo $t_data['id'] ?>" target="_blank">View Submitted <?php echo $exe1->num_rows; ?> Response(s)</a>
                            <?php } ?>
